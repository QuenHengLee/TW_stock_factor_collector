...
import utils
from CollectorFactory.CollectorAbstractClass import StockCollector
from Database.database_components import Company, Stock
...
class TWStockCollector(StockCollector):
    def __init__(self):
        self._config     = Config()
        self._db         = Database()
        self._logger     = logs.setup_loggers("TWStockCollector") 
    ...
    def get_company(self):
        """
        取得資料庫 Company 中的所有台灣公司
        參數 : 無
        回傳值 : generator
        """
        db_session = self._db.db_session()
        companys = db_session.execute(
            select(Company).where(Company.country == "TW"))
    ...
    def get_data(self, symbol=None):
        ...
        if symbol is None:
            self._logger.error(
                "[CollectorFactory][TWCollectorFactory][TWStockCollector]::未輸入股票代號")
            return None
        ...
        excel.Visible = True
        wb_stock = excel.Workbooks.Open(os.getcwd()+"/data/stock.xlsm")
        ...
        # 執行巨集程式
        excel.Application.Run("stock.xlsm!refresh")
        wb_tej.Close()
        ...
        for symbol in symbols:
            ...
            while not isDone:
                ...
                if "TejRefresh" in process_list:
                    ...
                    isDone = True
            self._logger.warning("start "+ str(symbol))
